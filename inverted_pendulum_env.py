...
class InvertedPendulumEnv(gym.Env):
    ...
    def step(self, action):
        ...
        rewards = {
            "theta_dot":-0.25, # Reward for small theta_dot
        ...
        if 90 - self.margin <= theta <= 90 + self.margin:
            reward += rewards["theta"] 

            # Penalize for deviation of x from the center (self.groove_length / 2)
            target_x = self.groove_length / 2  
            position_deviation = np.abs(x - target_x) / (self.groove_length / 2)  # Normalize deviation
            #reward -= position_deviation*0.2 # Penalize for being far from the target x position
